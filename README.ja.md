...
対話ウィザードが起動し、設定の書き込み先（プロジェクトの `.mcp.json` かユーザー全体の
`~/.claude.json`）、サーバー名、ワークスペースルート、ツールロスター（すべて / `core`
プロファイル / カテゴリ手動選択）、出力フォーマット、ダッシュボード設定、機能フラグを
順に尋ね、最後に書き込む JSON をプレビューします。`t0k3n setup --yes`（および非 TTY 実行）
はすべての質問をスキップしてデフォルトを書き込みます。
...
| フィーチャー | デフォルト | 効果 |
|--------------|------------|------|
| `documents` | 有効 | `convert_document`（PDF/DOCX → Markdown）。`pdf-extract` と `docx-rs` を取り込む |

配布バイナリはデフォルトフィーチャー有効。`cargo build --release --no-default-features` で
両パーサーをバイナリから外せる。その場合 `convert_document` は呼び出し時に失敗するのではなく
そもそも登録されない。どのツールが省かれているかは `--list-tools` と `debug_info` が報告する。
