...
### Phase 22 — ツールスキーマのコスト削減（v4.0 準備）

「ツールが多すぎるのではないか」という問いに対し、まず実測した。デフォルト 79 ツールの
`tools/list` ペイロードは 52,713 文字（≒15k トークン）で、200k コンテキストの 7.5% を
起動時点で占める。読み込みの 87% を削ると謳う製品としては説明責任のある数字なので、
体感ではなく実測を起点に削る。

計測内訳（文字数・コンパクト直列化）。当初は生 JSON への正規表現で内訳を出したが、
ツール説明とフィールド説明が同じ `"description":` キーを使うため二重計上していた。
スキーマを実際に walk して数え直した値が以下:

| 区分 | 文字数 | 比率 |
|---|---:|---:|
| ツール説明（79 個） | 14,623 | 28% |
| フィールド説明（184 個） | 11,859 | 22% |
| 構造 JSON（キー名・型・`required` 等） | 約 19,700 | 37% |
| `$schema` キー（79 個） | 4,029 | 8% |
| `title: "XxxParams"`（78 個） | 2,310 | 4% |
| `nullable: true`（105 個） | 1,575 | 3% |

- [x] **A-1 スキーマ痩身**: `src/server/schema_slim.rs` を追加。ルーター構築後・ケイパビリティ
  ゲート適用後に、登録済み全ツールの `input_schema` から `$schema` / `title` / `nullable` を
  再帰除去する。前二者は schemars のデフォルト出力であって MCP クライアントは参照しない。
  `nullable` は OpenAPI 3.0 の語彙で draft-07 のキーワードではないためバリデータは元から
  無視しており、かつ対象は例外なく `required` に無い `Option<T>` なので、draft-07 が
  `required` で表現している任意性を 1.5k 文字かけて言い直しているだけだった。
  実測 52,713 → 44,515 文字（**-15.6%**、約 2,300 トークン）。検証セマンティクスは不変
  - 落とし穴: `properties` / `definitions` の直下はキーが**引数名**であり、素朴に
    `title` を消すと `task_create` の `title` 引数が消滅する。名前マップ
    （`properties` `definitions` `$defs` `patternProperties`）とスキーマキーワードを
    区別し、インスタンス値（`default` `const` `enum` `examples`）には降りない
  - `served_schemas_carry_no_schemars_boilerplate` で、単体関数ではなく
    **サーバーが実際に配るスキーマ**に対して回帰を張る。この種の劣化は何も壊さず
    静かにコンテキストを食うだけなので、テストがなければ再混入に気づけない
- [x] **B core プロファイル**: `--tools` のカテゴリ指定は Phase 既存で実装済みだったため、
  残作業は名前付きプロファイルの解決のみ。`TOOL_PROFILES` に `core = file,git,text,debug`
  を定義し、`expand_profile()` でトークンを展開する。`known_tool_categories()` が
  プロファイル名も返すことで `--tools core` が起動時バリデーションを通る
...
- [x] **A-2 説明文の圧縮 — 見送り**。当初は「定型句の重複を削れば大きい」と見込んだが、
  ...
  **382 文字**（最大でも "Root-relative path to the code file" の 5 回・140 文字）しかない。
  残るツール説明 14.6k 文字は定型句ではなく、「行番号が陳腐化したら skeleton を取り直せ」
  「dry_run は書かずに差分だけ返す」といった運用知識そのもので、削れば減るのは
  トークンではなくツール選択の精度になる。これは「ツールが多すぎてモデルが選び違える」
