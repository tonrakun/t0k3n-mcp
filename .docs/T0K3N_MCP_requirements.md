...
| 項目 | 仕様 |
|---|---|
| 取得元 | 環境変数 `ANTHROPIC_API_KEY`（必須） |
| 設定ファイル | 不使用。ファイルへの書き込み・読み込みは行わない |
| `--root` との関係 | API キーは `--root` 配下のファイルとは完全に独立。T0K3N-MCP ツール経由では一切アクセス不可 |
| 未設定時の挙動 | ツール呼び出し時に `ANTHROPIC_API_KEY が未設定です` エラーを返し、処理を中断 |
| ログ出力 | API キーを標準出力・ログファイルに出力しない |

> **セキュリティ注意**: `semantic_search` はサブプロセスとして Claude API を呼び出すため、`--root` 外への通信が発生する。これは意図的な仕様であり、ネットワーク接続を許可しない環境では利用不可。
