...
| 区分 | 文字数 | 比率 |
|---|---:|---:|
| フィールド説明（263 個） | 30,736 | 58% |
| ツール説明（79 個） | 14,623 | 28% |
| `$schema` キー（79 個） | 4,029 | 8% |
| `title: "XxxParams"`（78 個） | 2,310 | 4% |
| `nullable` / `format` / `minimum` | 2,371 | 5% |

- [x] **A-1 スキーマ痩身**: `src/server/schema_slim.rs` を追加。ルーター構築後・ケイパビリティ
  ゲート適用後に、登録済み全ツールの `input_schema` から `$schema` と `title` を再帰除去する。
  どちらも schemars のデフォルト出力であって MCP クライアントは参照しない。
  実測 52,713 → 46,195 文字（**-12.4%**、約 1,850 トークン）。検証セマンティクスは不変
...
- [x] **B core プロファイル**: `--tools` のカテゴリ指定は Phase 既存で実装済みだったため、
  ...
  |---|---:|---:|
  | デフォルト | 79 | 46,195 文字 |
  | `--tools core` | 33 | 19,544 文字（**-58%**） |
  | `--enable-writes` | 90 | 56,483 文字 |

- [ ] **A-2 説明文の圧縮**: 残る 86% はツール説明とフィールド説明。`help()` カタログが
  詳細説明の受け皿になっているので、`tools/list` 側は「どのツールを選ぶか」の判断に
  必要な最小限へ寄せる
