...
pub struct SymbolEdit {
    #[schemars(
        description = "Exact text to find within the symbol's current source. Must match exactly once at the time this edit is applied."
    )]
    pub find: String,
...
pub struct PatchSymbolParams {
    ...
    pub id: String,
    #[schemars(
        description = "Replacement text for the entire line range of the symbol (signature + body, no trailing newline needed). Provide exactly one of new_body or edits."
    )]
    pub new_body: Option<String>,
    #[schemars(
        description = "Partial edits applied inside the symbol's current source, in order. Each find only needs to be unique within the symbol, not the file. Prefer this over new_body for small changes — no need to resend unchanged lines."
    )]
    pub edits: Option<Vec<SymbolEdit>>,
    #[schemars(
        description = "Symbol name expected inside the replaced range. Strongly recommended: the patch is rejected if the name is not found there, which catches stale line numbers."
    )]
    pub expected_name: Option<String>,
    #[schemars(
        description = "true = return the would-be diff without writing the file (default: false)"
    )]
...
pub fn patch_symbol(root: &Path, params: PatchSymbolParams) -> anyhow::Result<PatchSymbolResult> {
    ...
    let (kind, start, end) = parse_id(&params.id).ok_or_else(|| {
        anyhow::anyhow!(
            "invalid id '{}' — expected format 'kind:start-end'",
            params.id
        )
    })?;
