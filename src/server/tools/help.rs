...
pub(crate) fn catalog() -> BTreeMap<&'static str, Vec<ToolEntry>> {
    ...
    cat!(
        ...
        [
            ...
            (
                "read_file_outline",
                "auto-detect file type and return outline"
            ),
            ...
            (
                "read_type_skeleton",
                "TS/Go/Rust type definitions with fields"
            ),
            (
                "read_call_graph",
                "callers/callees; depth>=1 for cross-file"
            ),
            ("read_token_map", "largest files first by token count"),
            (
                "read_workspace_stats",
                "language breakdown across workspace"
            ),
            (
                "read_interface_conformance",
                "types implementing interface/trait"
            ),
            ("search_file", "keyword/regex search with context lines"),
            ("semantic_search", "natural language → relevant code bodies"),
            (
                "read_context_pack",
                "one-call task context: ranked files+symbols+bodies within budget"
            ),
            (
                "read_symbol_usages",
                "all usages of a symbol across workspace"
            ),
    ...
    cat!(
        ...
        [
            ...
            (
                "rename_symbol",
                "rename a symbol workspace-wide (write counterpart of read_symbol_usages)"
            ),
            ("create_file", "create a new file (opt-in: --enable-writes)"),
            (
                "delete_symbol",
                "delete a symbol by ID — pairs with read_dead_code (opt-in: --enable-writes)"
            ),
            (
                "insert_symbol",
                "insert code at a structural location (opt-in: --enable-writes)"
            ),
            (
                "apply_edits",
                "atomic multi-file find/replace — pairs with batch_read (opt-in: --enable-writes)"
            ),
            (
                "set_config_value",
                "set a JSON/YAML/TOML value by dot-path — pairs with read_json_yaml_value (opt-in: --enable-writes)"
            ),
            (
                "manage_imports",
                "add/remove import statements, dedup (opt-in: --enable-writes)"
            ),
            (
                "format_code",
                "run rustfmt/prettier/black/gofmt on a file (opt-in: --enable-writes)"
            ),
            (
                "move_symbol",
                "move a symbol to another file; warns on referencing files (opt-in: --enable-writes)"
            ),
            (
                "edit_checkpoint",
                "snapshot the working tree before edits — git stash or copy (opt-in: --enable-writes)"
            ),
            (
                "rollback",
                "restore the working tree to an edit_checkpoint (opt-in: --enable-writes)"
            ),
            (
                "write_markdown_section",
                "edit a Markdown section by heading anchor — pairs with read_markdown_toc/section (opt-in: --enable-writes)"
            ),
        ]
    );
    cat!(
        ...
        [
            (
                "read_git_diff",
                "compressed diff vs HEAD or any ref (zoom: skeleton/sketch/body/auto)"
            ),
            ...
            (
                "read_code_ownership",
                "churn + per-author line share + last-modified (git log+blame)"
            ),
        ]
    );
    cat!(
        ...
        [
            ...
            (
                ...
                "npm/cargo/go/python/maven/gradle unified"
            ),
            (
                "read_ci_pipeline",
                "GitHub Actions/GitLab/CircleCI structure"
            ),
        ]
    );
    cat!(
        ...
        [
            ...
            (
                ...
                "section from cached webpage by anchor"
            ),
    ...
    cat!(
        ...
        [
            (
                "read_test_skeleton",
                "test suite structure (Jest/pytest/Cargo/Go)"
            ),
            ("read_test_results", "parse test runner output to summary"),
            (
                "read_test_coverage",
                "map coverage (lcov/coverage.py/cobertura) onto symbols"
            ),
        ]
    );
    cat!(
        ...
        [
            ...
            (
                "read_stack_trace",
                "stack trace → source context resolution"
            ),
        ]
    );
    cat!(
        ...
        [
            ...
            (
                "delta_reset",
                "reset delta-read ledger (full content on next read)"
            ),
    ...
    cat!(
        ...
        [
            ...
            (
                ...
                "injection/XSS/secrets/unsafe patterns"
            ),
            (
                "read_dependency_audit",
                "dependency vulnerability scan (cargo/npm/pip/osv audit)"
            ),
            (
                "read_api_surface",
                "public API surface (pub/export/__all__/Go caps)"
            ),
            (
                "diff_schemas",
                "schema diff between git refs (OpenAPI/Prisma/TS)"
            ),
            (
                "read_pr_context",
                "full PR context: files+skeletons+tests+commits"
            ),
            (
                "read_type_diagnostics",
                "LSP-equivalent type errors (cargo check/tsc/pyright/go vet) — opt-in: --enable-diagnostics"
            ),
            (
                "project_digest",
                "cached warm-start architecture summary (HEAD-invalidated)"
            ),
        ]
    );
    cat!(
        ...
        [(
            "run_command",
            "run shell command with smart output filtering"
        ),]
    );
    cat!(
        "debug",
