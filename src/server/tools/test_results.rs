...
pub struct ReadTestResultsParams {
    #[schemars(description = "Root-relative path to a test output file. Use instead of 'text' for file-based output.")]
    pub path: Option<String>,
    #[schemars(description = "Raw test output text (paste from terminal). Use instead of 'path'.")]
    pub text: Option<String>,
    #[schemars(description = "Test framework hint: 'jest', 'vitest', 'pytest', 'cargo', 'go', 'auto' (default: auto).")]
...
pub fn read_test_results(root: &Path, params: ReadTestResultsParams) -> anyhow::Result<ReadTestResultsResult> {
    ...
    Ok(ReadTestResultsResult { framework, summary, suites, failures, token_count })
}
...
fn parse_jest(text: &str) -> (TestSummary, Vec<TestSuiteEntry>, Vec<TestFailure>) {
    ...
    let re_summary = Regex::new(
        r"Tests:\s+(?:(\d+) failed(?:,\s*)?)?(?:(\d+) skipped(?:,\s*)?)?(\d+) passed"
    ).unwrap();
    let re_duration = Regex::new(r"Time:\s+([\d.]+)\s*s").unwrap();

    let flush_suite = |suite: &str, p: usize, f: usize, s: usize, fl: &[String]| -> TestSuiteEntry {
        TestSuiteEntry {
    ...
    for line in text.lines() {
        if let Some(cap) = re_suite.captures(line) {
            if !current_suite.is_empty() {
                suites.push(flush_suite(&current_suite, suite_passed, suite_failed, suite_skipped, &suite_failures));
            }
            current_suite = cap[2].to_string();
            suite_passed = 0; suite_failed = 0; suite_skipped = 0;
            ...
            failures.push(TestFailure { name, suite: current_suite.clone(), message: String::new() });
    ...
    let summary = TestSummary { total, passed: total_passed, failed: total_failed, skipped: total_skipped, duration_ms };
...
fn parse_pytest(text: &str) -> (TestSummary, Vec<TestSuiteEntry>, Vec<TestFailure>) {
    ...
    let re_summary = Regex::new(
        r"=+\s+(?:(\d+) failed[,\s])?(?:(\d+) passed[,\s])?(?:(\d+) skipped[,\s])?"
    ).unwrap();
    ...
    for line in text.lines() {
        if let Some(cap) = re_test.captures(line) {
            ...
            let status = &cap[3];
            let entry = suite_map.entry(suite.clone()).or_insert_with(|| TestSuiteEntry {
                name: suite.clone(), passed: 0, failed: 0, skipped: 0, failed_tests: Vec::new(),
            });
            match status {
                ...
                "FAILED" | "ERROR" => {
                    ...
                    failures.push(TestFailure { name: test_name, suite, message: String::new() });
                }
                _ => entry.skipped += 1,
            }
        } else if let Some(cap) = re_summary.captures(line) {
...
fn parse_cargo(text: &str) -> (TestSummary, Vec<TestSuiteEntry>, Vec<TestFailure>) {
    ...
    let re_result = Regex::new(
        r"test result:.*?(\d+) passed; (\d+) failed; (\d+) ignored;.*?([\d.]+)s"
    ).unwrap();

    for line in text.lines() {
        if let Some(cap) = re_running.captures(line) {
            current_suite = cap.get(1)
                .map(|m| m.as_str().to_string())
                .unwrap_or_else(|| "default".to_string());
        } else if let Some(cap) = re_test.captures(line) {
            let test_name = cap[1].to_string();
            let entry = suite_map.entry(current_suite.clone()).or_insert_with(|| TestSuiteEntry {
                name: current_suite.clone(), passed: 0, failed: 0, skipped: 0, failed_tests: Vec::new(),
            });
            match &cap[2] {
                "ok" => entry.passed += 1,
                "FAILED" => {
                    entry.failed += 1;
                    entry.failed_tests.push(test_name.clone());
                    failures.push(TestFailure { name: test_name, suite: current_suite.clone(), message: String::new() });
                }
                _ => entry.skipped += 1,
            }
    ...
    let summary = TestSummary { total, passed: total_passed, failed: total_failed, skipped: total_ignored, duration_ms };
    (summary, suites, failures)
}
...
fn parse_go_test(text: &str) -> (TestSummary, Vec<TestSuiteEntry>, Vec<TestFailure>) {
    ...
    for line in text.lines() {
        if let Some(cap) = re_pkg.captures(line) {
            ...
            suite_map.entry(current_pkg.clone()).or_insert_with(|| TestSuiteEntry {
                name: current_pkg.clone(), passed: 0, failed: 0, skipped: 0, failed_tests: Vec::new(),
            });
            let _ = cap; // suppress unused warning
        } else if let Some(cap) = re_test.captures(line) {
            let status = &cap[1];
            let test_name = cap[2].to_string();
            let entry = suite_map.entry(current_pkg.clone()).or_insert_with(|| TestSuiteEntry {
                name: current_pkg.clone(), passed: 0, failed: 0, skipped: 0, failed_tests: Vec::new(),
            });
            if status == "PASS" {
                entry.passed += 1;
                ...
                entry.failed += 1;
                entry.failed_tests.push(test_name.clone());
                failures.push(TestFailure { name: test_name, suite: current_pkg.clone(), message: String::new() });
            }
        }
    }
...
fn parse_generic_test(text: &str) -> (TestSummary, Vec<TestSuiteEntry>, Vec<TestFailure>) {
    ...
    let summary = TestSummary { total: passed + failed, passed, failed, skipped: 0, duration_ms: None };
